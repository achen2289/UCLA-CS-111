...
const char ssl_init_error[] = "ERROR: Could not set up SSL";
const char ssl_command_error[] = "ERROR: SSL communication could not occur";
...
void shut_down() {
    ...
    int status = SSL_write(ssl_client, report, strlen(report)); //send to server
    check_return_error(status, ssl_command_error, 2);
...
void initialize_SSL() {
    ...
    if (new_context == NULL) {
        check_return_error(-1, ssl_init_error, 2);
    }
...
void send_ID() {
    ...
    int status = SSL_write(ssl_client, id_str_record, strlen(id_str_record));
    check_return_error(status, ssl_command_error, 2);
}
...
void print_record() {
    ...
    int status = SSL_write(ssl_client, report, strlen(report));
    check_return_error(status, ssl_command_error, 2);
...
void read_input() {
    ...
    while (42069) {
        ...
        if (polls > 0 && (pollfds[0].revents & POLLIN)) {
            char buffer[1024] = {0};
            ssize_t bytes_read = SSL_read(ssl_client, buffer, 1024);
            check_return_error(bytes_read, ssl_command_error, 2);
